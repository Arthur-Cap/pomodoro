...
function getTask() {
  ...
  time[0].value.length < 2
    ? (time[0].value = "0" + time[0].value)
    : (time[0].value = time[0].value);
  time[1].value.length < 2
    ? (time[1].value = "0" + time[1].value)
    : (time[1].value = time[1].value);
  time[2].value.length < 2
    ? (time[2].value = "0" + time[2].value)
    : (time[2].value = time[2].value);
...
function getValue() {
  setInterval(() => {
    ...
    document.getElementById("focusTime").innerHTML =
      convertSecondsToHMS(focusTime + 1) + " s";
  }, 1000);
}
...
function addTask(taskChoose) {
      ...
      for (let i = 0; i < taskList.length; i++) {
        ...
        if (getTotalSec(choose) < getTotalSec(taskList[i])) {
          document
            ...
            .insertBefore(div1, document.getElementById(riuID));
          flag = 1;
        }
      }
      if (flag == 0) {
  ...
  buton.addEventListener("click", function () {
    ...
    setTimeout(function () {
      ...
      if (counting > 5) {
        taskList.splice(coutingRemove - 1, 1);
      } else {
...
function showwTask() {
  ...
  taskDetails.forEach(function (taskDetail) {
    taskDetail.addEventListener("click", function (e) {
      document.querySelector("#page2-top").style.height = 0;
      document.querySelector("#page2-top").style.transform =
        "translateY(-100vh)";
      document.querySelector("#page2-bot").style.marginTop = 0;
      setTimeout(function () {
        document.querySelector("#page2-bot_close-btn").style.transform =
...
function extractPlaylistId(playlistLink) {
  var regex =
    /(?:https?:\/\/(?:www\.)?youtube\.com\/playlist\?list=|https?:\/\/(?:www\.)?youtu\.be\/)([a-zA-Z0-9_-]{10,})/;
...
async function fetchData() {
  try {
    chillMusic = await getVideoIds(
      "https://youtube.com/playlist?list=PLnAH_52EwoIXQ2ZAFNM2YHBUkvhic0Dfb"
    );
    whiteNoise = await getVideoIds(
      "https://youtube.com/playlist?list=PLnAH_52EwoIWwgJhlT_Ts2_EOYZ0Ha-pR"
    );
    electronicMusic = await getVideoIds(
      "https://youtube.com/playlist?list=PLnAH_52EwoIXSj_xUxZX95plTTi2rhRvV"
    );
    instrumentalMusic = await getVideoIds(
      "https://youtube.com/playlist?list=PLnAH_52EwoIVgpPDZCqx41Z9nPL5eIPew"
    );
